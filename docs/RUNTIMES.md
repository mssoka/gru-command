...
`compact()` (native in-place compaction), and `isCompacting()`. These are
not inferred capability booleans: presence plus live state determines the
chat snapshot. Fallback wrappers forward them while preserving their
...
- **Pi:** usage comes directly from `AgentSession.getContextUsage()` and
  ...
  events map to runtime `compaction_start/end`; the adapter verifies the
  session id/file did not change.
...
Both adapters reject controls while a turn/control is active. New chat is
not an adapter `reset`: orchestration mints a wholly fresh unresumed handle
and atomically swaps it at the durable chat boundary.
