...
export function renderReferenceConfig(
  ...
  lines.push(
    ...
    `enabled = ${preserved?.lessons.enabled ?? true}`,
    '# Dream cadence: on boot plus every interval (ms); 0 = no periodic pass.',
    `dream_interval_ms = ${preserved?.lessons.dreamIntervalMs ?? 43_200_000}`,
    `dream_on_boot = ${preserved?.lessons.dreamOnBoot ?? true}`,
    ...
    `chapter_cap_bytes = ${preserved?.lessons.chapterCapBytes ?? 4_096}`,
    `index_cap_bytes = ${preserved?.lessons.indexCapBytes ?? 1_024}`,
    '# Maximum pointer lines injected into one briefing/directive.',
    `max_references = ${preserved?.lessons.maxReferences ?? 3}`,
    '',
    '[silas]',
    ...
    `poll_interval_ms = ${preserved?.silas.pollIntervalMs ?? 60_000}`,
    ...
    `drain_timeout_ms = ${preserved?.roll.drainTimeoutMs ?? 900_000}`,
    '',
    '[review]',
