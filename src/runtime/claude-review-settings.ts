...
export function resolveClaudeReviewConfiguration(modelRef: string, file: string): ClaudeReviewConfiguration {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') return { modelRef, settings: {} };
  ...
  return {
    modelRef: (modelRef === '' || modelRef === 'default') && typeof model === 'string' ? model : modelRef,
    settings: {
...
export function privateClaudeReviewSettings(settings: ReviewSettings): { readonly file?: string; readonly dispose: () => void } {
  if (settings.apiKeyHelper === undefined && settings.env === undefined) return { dispose: () => {} };
